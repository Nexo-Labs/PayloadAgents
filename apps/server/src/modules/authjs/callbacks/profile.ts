...
export async function profileCallback(
  ...
  if (profile.organizations_info && typeof window === "undefined") {
    const keycloakOrgIds = parseKeycloakOrganizations(
      profile.organizations_info
    );
      ...
      setTimeout(async () => {
        try {
          ...
          if (user) {
            ...
            const result = await syncUserTenants(
              payload,
              String(user.id),
              keycloakOrgIds,
              { isOrgAdmin }
            ...
            if (result.assigned > 0) {
              ...
              console.log(
                `[Auth] ✅ Assigned user ${profile.email} to ${result.assigned} tenants as ${role}`
              );
            }
          }
        } catch (error) {
