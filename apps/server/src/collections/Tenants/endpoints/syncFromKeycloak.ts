...
async function getKeycloakAdminToken(): Promise<string> {
  ...
  if (!response.ok) {
    ...
    throw new APIError(
      `Failed to get Keycloak admin token: ${error}`,
      500,
      null,
      true
    );
...
async function getKeycloakOrganizations(
  accessToken: string
  ...
  handler: async (req) => {
    ...
    try {
      ...
      for (const org of organizations) {
        console.log(
          `[Keycloak] Processing org: ${org.name} (${org.id}), alias: ${org.alias}`
        );
        ...
        try {
          ...
          if (tenant) {
            console.log(
              `[Keycloak] Found existing tenant by keycloakOrgId: ${tenant.name}`
            );
            ...
            if (tenant.name !== org.name) {
              ...
              console.log(
                `[Keycloak] ⏭️ Skipped tenant: ${org.name} (no changes)`
              );
            }
          } else {
            ...
            if (tenantBySlug) {
              console.log(
                `[Keycloak] Found existing tenant by slug: ${tenantBySlug.name}`
              );
              ...
              results.updated.push(org.name);
              console.log(
                `[Keycloak] ✅ Updated tenant with keycloakOrgId: ${org.name}`
              );
            } else {
              console.log(
                `[Keycloak] Creating new tenant: ${org.name}, slug: ${org.alias}`
              );
              // Create new tenant
